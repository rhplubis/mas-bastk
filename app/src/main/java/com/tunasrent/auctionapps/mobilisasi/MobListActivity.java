...
import com.tunasrent.auctionapps.model.Bplist;
import com.tunasrent.auctionapps.model.BplistAdapter;
import com.tunasrent.auctionapps.model.Moblist;
import com.tunasrent.auctionapps.model.MoblistAdapter;
...
public class MobListActivity extends AppCompatActivity implements AdapterView.OnItemClickListener {
    ...
    private ArrayList<Moblist> item_list = new ArrayList<>();
...
//                if (key.equals("title")){
//                    Log.d("login_ttl",getIntent().getExtras().getString(key));
//                }else if (key.equals("message")){
//                    Log.d("login_msg",getIntent().getExtras().getString(key));
//                }else if (key.equals("message")){
                  //  Toast.makeText(MobListActivity.this,getIntent().getExtras().getString("title"),Toast.LENGTH_SHORT).show();
//                }
            //}
      //  }
...
//                hashMap.put(configuration.KEY_PARENT,"0");
                ...
                hashMap.put(configuration.KEY_POOL,_pool);
        ...
        try {
            if (code.equals("200")) {
                for (int i = 0; i < result.length(); i++) {
                    ...
                    Moblist moblist = new Moblist(vhcdate, vhcid, vhctype, vendor, status, procby,type_aset);

                    item_list.add(moblist);
                    ...
                    mAdapter = new MoblistAdapter(item_list);
                    rvDetail.setAdapter(mAdapter);
                    rvDetail.getAdapter().notifyDataSetChanged();
                }

            } else if (code.equals("304")) {
                Toast.makeText(MobListActivity.this, message, Toast.LENGTH_SHORT).show();

                Intent intent = new Intent(MobListActivity.this, LoginActivity.class);
